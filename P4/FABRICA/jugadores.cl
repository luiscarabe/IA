...
;; Nuestra primera heuristica va a ser pesima

(defun f-eval-Heur1 (estado)
   (valorar-Heur1 (reverse (list-lado estado (estado-lado-sgte-jugador estado))) 0 10))

(defun valorar-Heur1 (lista contador index)
  (cond ((null lista)
         contador)
        ((eql (first lista) 0) 
         (valorar-Heur1 (rest lista) (+ contador index) (- index 1)))
        (t (valorar-Heur1 (rest lista) contador (- index 1)))))

(defparameter *jdr-pesimo* (make-jugador
                      :nombre '|pumba|
                      :f-juego #'f-j-nmx
                            :f-eval #'f-eval-Heur1))
